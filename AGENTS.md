...
- Each job remains pinned to its registered staging scope by `StorageID`; new jobs share a canonical mount-root scope when the mount root is writable, otherwise they reuse or create a same-mount scope beside the target. Removed jobs and staging jobs without an execution binding emit no startup artifact; staging cleanup validates its registered marker independently so a renamed or missing final target cannot prevent safe task removal.
...
- Dashboard rows, details, ownership, issues, actions, and public status are app-owned and rendered verbatim by the TUI: native rows are joined by current execution GID but rewritten to stable JobID before crossing the app contract. Native `active` is refined to `downloading`, `metadata`, or `seeding`; bounded native batch reads and the asynchronous single-flight refresh contract remain unchanged. See `docs/implemented/dashboard-read-model-ownership.md` and `docs/implemented/unified-dashboard-status.md`.
