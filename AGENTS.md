...
- `app.ReconcileJob` is the sole managed lifecycle convergence owner. Add persists a distinct random execution GID before RPC; Pause and Resume persist activity intent, while Remove persists `Removed=true`, retires native state, cleans unpublished staging, preserves published payloads, and deletes the manifest on success. Failed removal remains an Error with Remove as its only action and Retry never revives it; startup finishes the same transaction after saved-block omission proves native retirement. See `docs/implemented/unified-remove-task.md`.
...
- macOS SMB storage scopes registered while online retain a credential-free Finder reconnect URL; only explicit Retry may request the missing mount, outside the job lock, and the existing storage rebinding boundary remains the sole authority that accepts the mounted filesystem. Linux and scopes without a reconnect URL retain their existing behavior. See `docs/implemented/retry-network-storage-remount.md`.
...
- Managed runtime logging suppresses aria2's interactive console readout and summaries; `managed-exec` owns startup-time 50 MiB rotation and fd binding for stdout/stderr, retains the active file plus `.1` and `.2`, and rejects unsafe log paths. Supervisors must not pre-open these files. See `docs/implemented/bounded-managed-logs.md`.
...
- `app.ProjectTask` is the sole Dashboard status/ownership/issue/action policy: it consumes explicit native, manifest, identity, and filesystem-capability facts without performing I/O. User actions are `pause`, `resume`, `retry`, `reseed`, and `remove`; native/managed removal phases are internal, and the TUI derives state-specific Help from these capabilities. Durable issue presentation and action overrides derive from the `jobs` catalog; a detached prepared payload synthesizes publication recovery, while a corrupt manifest remains diagnostic-only because its native ownership cannot be proven.
- Dashboard rows, details, ownership, issues, actions, and public status are app-owned and rendered verbatim by the TUI: native rows use compact fields, hydrate full file identity only when a torrent name is unavailable, join by current execution GID, and cross the app contract under stable JobID; manifest-only published details restore file facts from retained torrent metainfo or the single payload root. Native `active` is refined to `downloading`, `metadata`, or `seeding`; the TUI caches every successful detail by stable JobID, refreshes row-owned live fields each poll, and revalidates full details after a 10-second freshness window or a successful mutation. See `docs/implemented/dashboard-read-model-ownership.md`, `docs/implemented/unified-dashboard-status.md`, `docs/implemented/dashboard-startup-read-path.md`, and `docs/implemented/dashboard-detail-cache.md`.
...
- **Application workflows, Dashboard contract, and composition**: `internal/app/app.go`, `internal/app/dashboard.go`, `internal/app/readmodel.go`, `internal/app/storage_reconnect.go`
...
- **Lifecycle reconciliation, publication boundary, and managed runtime**: `internal/app/lifecycle.go`, `internal/app/managedexec.go`, `internal/publication/publication.go`, `internal/runtime/runtime.go`, `internal/runtime/logs.go`
