...
- A prepared staging payload (`Payload.FinalRoot` set) is projected as publication recovery when native ownership is absent; Dashboard reads never execute reconciliation. Issue severity, text, and actions derive from one code table rather than durable presentation fields.
...
- **Lifecycle reconciliation, publication boundary, and managed runtime**: `internal/app/lifecycle.go`, `internal/app/managedexec.go`, `internal/publication/publication.go`, `internal/runtime/runtime.go`
