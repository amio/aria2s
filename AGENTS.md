...
- `internal/upgrade` owns latest-release resolution, mandatory checksum verification, bounded candidate execution, and atomic CLI replacement; release identity comes from ldflags or Go module build metadata, GoReleaser publishes a raw platform binary for this path while retaining archives for the installer, and Homebrew-owned binaries remain package-manager controlled.
