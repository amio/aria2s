...
- Dashboard public status is app-owned and rendered verbatim by the TUI: native `active` is refined to `downloading`, `metadata`, or `seeding`; a completed managed metadata fetch remains `metadata` until payload promotion; otherwise `waiting`, `paused`, `complete`, `error`, and `removed` retain aria2 names. Terminal output failure cancels the Dashboard session so a detached renderer cannot spin. See `docs/implemented/unified-dashboard-status.md`.
