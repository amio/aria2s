...
- Dashboard public status is app-owned and rendered verbatim by the TUI: native `active` is refined to `downloading`, `metadata`, or `seeding`, while `waiting`, `paused`, `complete`, `error`, and `removed` retain aria2 names; terminal output failure cancels the Dashboard session so a detached renderer cannot spin. See `docs/implemented/unified-dashboard-status.md`.
