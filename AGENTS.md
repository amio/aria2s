...
- Each job remains pinned to its registered staging scope by `StorageID`; new jobs share a canonical mount-root scope when the mount root is writable, otherwise they reuse or create a same-mount scope beside the target. `Pending` and `Removed` jobs emit no startup artifact and cannot make service startup depend on storage inspection.
