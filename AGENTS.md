...
- `app.ReconcileJob` is the sole managed lifecycle convergence owner. Add persists a distinct random execution GID before RPC, commands persist only user intent, hooks resolve exactly one manifest by execution GID, and startup uses the exclusive process lease plus saved-block omission as authoritative native retirement; explicit Retry may create or adopt a same-path target only for unpublished staging work after physical-parent, registered-marker, and mount validation, while deleting a metadata-only task also clears its manifest after native and staging cleanup so it leaves no removed-history row.
