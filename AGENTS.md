...
- Durable job ownership, staging/final payload roots, publication intent, and final logical payload length live in versioned manifests; aria2's native v2 session remains the transport-resume artifact and is normalized into a fresh startup input before every managed exec. Removed tombstones suppress automatic startup, while explicit Retry safely restarts the same job after detach and cleanup. See `docs/implemented/completed-task-metrics.md`, `docs/implemented/removed-task-restart.md`, and `docs/implemented/automatic-publication-suffix.md`.
- Filesystem publication is one guarded, portable same-filesystem rename of a single payload root; `publication` owns path/identity/filesystem facts while `app` owns destination allocation and the detach/move/rehydrate transaction. Managed publications serialize briefly and auto-suffix observed conflicts; renamed torrents stop instead of seeding paths that differ from metainfo, while concurrent external target writers remain outside the guarantee.
...
- Dashboard public status is app-owned and rendered verbatim by the TUI: native `active` is refined to `downloading`, `metadata`, or `seeding`; a completed managed metadata fetch remains `metadata` until payload promotion; otherwise `waiting`, `paused`, `complete`, `error`, and `removed` retain aria2 names. The terminal output observer preserves Bubble Tea's TTY file contract, while output failure cancels the Dashboard session so a detached renderer cannot spin. See `docs/implemented/unified-dashboard-status.md`.
