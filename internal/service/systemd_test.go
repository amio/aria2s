...
func TestRenderSystemdUnitUsesAbsoluteAria2cPathWithoutShell(t *testing.T) {
	...
	assertContains(t, rendered, "ExecStart=/usr/bin/aria2c --enable-rpc=true --rpc-listen-all=false --rpc-listen-port=6800 --rpc-secret=secret-token --input-file=/home/amio/.local/state/aria2s/session --save-session=/home/amio/.local/state/aria2s/session --save-session-interval=60 --on-download-complete=/home/amio/.local/state/aria2s/on-complete-hook.sh")
