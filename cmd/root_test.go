...
// TestDoctorOutputPrintsDetailAndRemedy asserts the doctor command renders
// the enriched issue format (label, detail, → remedy) so users can act on
// each issue without reading docs.
func TestDoctorOutputPrintsDetailAndRemedy(t *testing.T) {
	rootDir := t.TempDir()
	servicePaths := paths.NewDarwin(filepath.Join(rootDir, "home"))
	aria2c := filepath.Join(rootDir, "missing-aria2c") // triggers "missing aria2c binary"
	current := state.State{
		Aria2cPath:   aria2c,
		RPCPort:      6800,
		RPCSecret:    "secret-token",
	...
	application := newTestApp(servicePaths, aria2c, &recordingService{}, &trackingRPC{})
	// Override port availability to force a port conflict as well.
	application = app.New(app.Options{
		Paths:           servicePaths,
	...
	output := out.String()
	assertContains(t, output, "- missing aria2c binary")
	assertContains(t, output, "→ Run `aria2s install` to reinstall aria2c.")
	assertContains(t, output, "- port conflict")
	assertContains(t, output, "→ Run `aria2s restart`.")
}
